...
# Separating explicit and implicit ratings into two separate dataframes
ratings_explicit = ratings.loc[ratings['Book-Rating'] != 0, :].copy()
ratings_implicit = ratings.loc[ratings['Book-Rating'] == 0, :].copy()
# Saving the cleaned datasets to new CSV files in dataset_mod folder
if not os.path.exists('./dataset_mod'):
    os.makedirs('./dataset_mod')
users.to_csv('./dataset_mod/users_mod.csv', index=False)
books.to_csv('./dataset_mod/books_mod.csv', index=False)
ratings_explicit.to_csv('./dataset_mod/ratings_explicit_mod.csv', index=False)
ratings_implicit.to_csv('./dataset_mod/ratings_implicit_mod.csv', index=False)
